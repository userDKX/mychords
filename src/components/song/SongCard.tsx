...
export function SongCard({ song, showAuthor, onRemoveSaved, overlay }: SongCardProps) {
  const hasOverlay = !!onRemoveSaved || !!overlay
  const creatorName = song.profiles?.display_name

  return (
    <div className="relative">
      <Link
        to={songPath(song)}
        className="group relative flex flex-col justify-between overflow-hidden bg-slate-900/40 border border-white/10 hover:border-brand-500/50 rounded-3xl p-6 transition-all duration-300 hover:-translate-y-1 hover:bg-slate-800/40"
      >
        ...
        <div className="relative z-10 flex-col flex h-full">
          <div className="flex items-start justify-between gap-4 mb-4">
            ...
            {/* Only show public/private icon when there's no overlay button occupying this corner */}
            {!hasOverlay && (
          ...
          <div className="mt-6 flex items-center justify-between pt-5 border-t border-white/10">
            ...
            {showAuthor && creatorName && (
              <span className="text-xs text-slate-500 bg-black/20 px-2 py-1 rounded-md font-medium truncate max-w-[120px]">
                Por {creatorName}
              </span>
            )}

            <div className="w-10 h-10 rounded-2xl bg-white/5 flex items-center justify-center text-slate-400 group-hover:bg-brand-500 group-hover:text-white transition-all duration-300 shadow-sm">
      ...
      {onRemoveSaved && (
        <button
          onClick={(e) => { e.preventDefault(); onRemoveSaved(song.id) }}
          className="absolute top-3 right-3 z-20 w-9 h-9 flex items-center justify-center rounded-xl bg-black/60 text-amber-400 hover:text-rose-400 hover:bg-rose-500/20 ring-1 ring-white/10 backdrop-blur-md transition-all duration-300"
          title="Quitar de guardados"
        >
          <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z" /></svg>
        </button>
      )}

      {/* Custom overlay (e.g. save button in community) */}
      {overlay}
    </div>
  )
}
